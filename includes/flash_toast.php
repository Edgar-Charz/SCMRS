...
?>
<div class="position-fixed top-0 start-50 translate-middle-x p-3" style="z-index:11000; width:min(680px, 92vw);"
    id="flashToastStack">
    <?php foreach ($_toasts as $_t):
        $bgType = explode(' ', $_t['type'])[0];
        $icon = $_iconMap[$_t['type']] ?? 'fa-info-circle';
        ?>
        <div class="toast show align-items-center text-white bg-<?= $_t['type'] ?> border-0 mb-2 shadow" role="alert"
