...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($responseText)) {
        ...
        try {
            $staff->respondToComplaint($complaintId, $userId, $responseText, $responseAction);
            $label = ($responseAction === 'resolve') ? 'resolved' : 'rejected';
            $_SESSION['message'] = "Complaint #$complaintId has been $label successfully.";
            header('Location: assigned_complaints.php');
            exit;
        } catch (Exception $e) {
            $error = $e->getMessage();
...
<head>
    ...
    <title>Respond — Complaint #<?= $complaintId ?> | Staff</title>
        ...
        <div id="content" class="w-100">
            ...
            <div class="p-4">
                ...
                <nav aria-label="breadcrumb" class="d-flex justify-content-between align-items-center mb-3">
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item">
                            <a href="staff_dashboard.php"><i class="fas fa-chart-pie" style="color: black;"></i></a>
                        </li>
                        <li class="breadcrumb-item">
                    ...
                    <a href="assigned_complaint_details.php?id=<?= $complaintId ?>" class="btn btn-add">
                ...
                <div class="container-card shadow-sm">
                    ...
                    <!-- Student info (hidden when anonymous) -->
                    <?php if (!$complaint['is_anonymous']): ?>
                        <div class="detail-row">
                            <div class="detail-label fw-bold">Student:</div>
                            <div class="detail-value"><?= htmlspecialchars($complaint['student_name']) ?></div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label fw-bold">Reg. Number:</div>
                            <div class="detail-value"><?= htmlspecialchars($complaint['student_registration_number'] ?? 'N/A') ?></div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label fw-bold">Email:</div>
                            <div class="detail-value"><?= htmlspecialchars($complaint['student_email'] ?? 'N/A') ?></div>
                        </div>
                        <?php if (!empty($complaint['student_phone'])): ?>
                            <div class="detail-row">
                                <div class="detail-label fw-bold">Phone:</div>
                                <div class="detail-value"><?= htmlspecialchars($complaint['student_phone']) ?></div>
                            </div>
                        <?php endif; ?>
                    <?php else: ?>
                        <div class="detail-row">
                            <div class="detail-label fw-bold">Student:</div>
                            <div class="detail-value"><em class="text-muted">Anonymous</em></div>
                        </div>
                    <?php endif; ?>
                ...
                <div class="container-card shadow-sm">
                    ...
                    <form method="POST" action="respond_assigned_complaint.php?id=<?= $complaintId ?>">
                        <?= csrf_field() ?>
